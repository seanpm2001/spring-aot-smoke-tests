...
public abstract class GenerateGitHubActionsWorkflow extends DefaultTask {
	...
	public GenerateGitHubActionsWorkflow() {
		getOutputFile().convention(getProject().getRootProject()
			.getLayout()
			.file(getSmokeTest().map((smokeTest) -> new File(
					".github/workflows/" + smokeTest.group() + "-" + smokeTest.name() + ".yml"))));
	}
